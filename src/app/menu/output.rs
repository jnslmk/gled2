...
impl App {
    pub fn config_output(&mut self, ui: &mut Ui) {
            ...
            .show(ui, |ui| {
                ui.with_layout(Layout::top_down_justified(egui::Align::Min), |ui| {
                    let mut new_outputs = None;
                    {
                        ...
                        ui.horizontal(|ui| {
                            if ui
                                .radio_value(&mut default_output_kind, OutputKind::Artnet, "Artnet")
                                .changed()
                            {
                            ...
                            Output::WledDRGB { ip, port } => {
...

                                {
                                        ...
                                        && port.parse::<u16>().is_ok()
                                    {
                                        ...
                                        *outputs.default_output_mut() = Output::WledDRGB {
                                            ip: *ip,
                                            port: port.parse().expect("Should never happen"),
                                        };
                                        new_outputs = Some(outputs);
                                    }
                                }
                                ...
                                UniverseOutput::Artnet { ip, universe } => {
...

                                    {
                                        ...
                                        let universe_str = self
                                            .inputs
                                            .entry(format!("artnet_universe_{universe}"))
                                            .or_insert_with(|| universe.to_string());
                                        if ui.add(TextEdit::singleline(universe_str)).changed()
