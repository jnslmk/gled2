...
impl App {
    pub fn menu(&mut self, ctx: &Context, frame: &Frame, ui: &mut Ui) {
        ...
        egui::menu::bar(ui, |ui| {
            ...
            let mut create_new_project = ctx.input_mut(|i| i.consume_key(Modifiers::CTRL, Key::N));
            ...
            let mut save_project_as =
                ctx.input_mut(|i| i.consume_key(Modifiers::CTRL.plus(Modifiers::SHIFT), Key::S));
            let mut open_svg_file =
                ctx.input_mut(|i| i.consume_key(Modifiers::CTRL.plus(Modifiers::SHIFT), Key::O));
