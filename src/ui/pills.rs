...
pub fn show_pills(ui: &mut Ui, start: Pos2, texts: Vec<String>) {
    const PADDING: f32 = 5.0;
    let mut pills = vec![];
    for text in texts {
        let text = egui::RichText::new(text).color(Color32::BLACK);
        let size = WidgetText::from(text.clone())
            .into_galley(ui, None, ui.available_width(), TextStyle::Body)
            .rect
            .size()
            + Vec2::new(4.0, 0.0);
        pills.push((text, size));
    }

    let total_width =
        pills.iter().map(|(_text, size)| size.x).sum::<f32>() + (pills.len() - 1) as f32 * PADDING;

    let mut start = start + Vec2::new(-total_width - 2.0, 0.0);
    for (text, size) in pills {
        let rect = Rect::from_min_size(start, size);
        ui.painter()
            .rect_filled(rect, Rounding::same(5.0), Color32::GOLD);
